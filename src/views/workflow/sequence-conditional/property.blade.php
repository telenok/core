<div class="modal-dialog">
    <div class="modal-content">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <form action="#" onsubmit="return false;" class="form-horizontal" id="model-ajax-{{$uniqueId}}">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="stencil[order]">{{$controller->LL('property.order')}}</label>
                                <div class="col-sm-9">
							...
							@foreach($property->get('condition', []) as $a)
								...
								@include('core::workflow.' . $controller->getKey() . '.condition-template')

							@endforeach
							
                            <button class="btn btn-info btn-xs add-condition pull-right" onclick="addContition{{$uniqueId}}(this); return false;">
                                <i class="ace-icon fa fa-plus-circle bigger-125"></i>
                                {{$controller->LL('btn.add.condition')}}</button>
        ...
        <script type="text/javascript">
            function addContition{{$uniqueId}}(dom_obj)
            {
                jQuery.ajax({
                    url: '{!! URL::route("cmf.workflow.sequence-conditional.conditional-template.content", []) !!}',
                    method: 'get',
