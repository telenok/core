...
class Controller extends \Telenok\Core\Abstraction\Field\Controller
{
    ...
    public function getModelSpecialAttribute($model, $key, $value)
    {
        if (in_array($key, ['decimal_default', 'decimal_min', 'decimal_max', 'decimal_precision', 'decimal_scale'], true) && $value === null) {
            ...
            } elseif ($key == 'decimal_min') {
                return \App\Vendor\Telenok\Core\Field\Decimal\BigDecimal::create('-9999999999999999999999999999', 2);
            } elseif ($key == 'decimal_max') {
                return \App\Vendor\Telenok\Core\Field\Decimal\BigDecimal::create('9999999999999999999999999999', 2);
            } elseif ($key == 'decimal_precision') {
                return 30;
            } elseif ($key == 'decimal_scale') {
                return 2;
            }
        }
    ...
    public function setModelSpecialAttribute($model, $key, $value)
    {
        if (in_array($key, ['decimal_default', 'decimal_min', 'decimal_max', true]) && $value instanceof \App\Vendor\Telenok\Core\Field\Decimal\BigDecimal) {
            $value = $value->value();
        } elseif (in_array($key, ['decimal_default', 'decimal_min', 'decimal_max', 'decimal_precision', 'decimal_scale'], true) && $value === null) {
            if ($key == 'decimal_default') {
                $value = 0;
            } elseif ($key == 'decimal_min') {
