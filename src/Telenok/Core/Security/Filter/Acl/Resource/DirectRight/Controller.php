<?php namespace Telenok\Core\Security\Filter\Acl\Resource\DirectRight;
    ...
    public function filterCan($queryCommon, $queryWhere, $resource, $permission, $subject)
    {
		...
		if ($subject instanceof \Telenok\Core\Model\User\User)
		{
			...
			$queryCommon->leftJoin($spr->getTable() . ' as spr_permission_user_filter_direct_right', function($join) use ($spr, $sequence, $permission, $r)
			{
				...
				$join->where('spr_permission_user_filter_direct_right.active_at_start', '<=', $r);
				$join->where('spr_permission_user_filter_direct_right.active_at_end', '>=', $r);
			}); 
			...
			$queryCommon->leftJoin($role->getTable() . ' as role_permission_user_filter_direct_right', function($join) use ($role, $r)
			{
				...
				$join->where('role_permission_user_filter_direct_right.active_at_start', '<=', $r);
				$join->where('role_permission_user_filter_direct_right.active_at_end', '>=', $r);
			}); 
			...
			$queryCommon->leftJoin($group->getTable() . ' as group_permission_user_filter_direct_right', function($join) use ($group, $r)
			{
				...
				$join->where('group_permission_user_filter_direct_right.active_at_start', '<=', $r);
				$join->where('group_permission_user_filter_direct_right.active_at_end', '>=', $r);
			}); 
			...
			$queryCommon->leftJoin($subject->getTable() . ' as user_permission_user_filter_direct_right', function($join) use ($subject, $r)
			{
				...
				$join->where('user_permission_user_filter_direct_right.active_at_start', '<=', $r);
				$join->where('user_permission_user_filter_direct_right.active_at_end', '>=', $r);
		...
		$queryCommon->leftJoin($spr->getTable() . ' as spr_filter_direct_right', function($join) use ($spr, $sequence, $subject, $permission, $r)
		{
			...
			$join->where('spr_filter_direct_right.active_at_start', '<=', $r);
			$join->where('spr_filter_direct_right.active_at_end', '>=', $r);
		}); 
        ...
        foreach($subjectCollection as $subject)
        {
            ...
            if ($subject instanceof \Telenok\Core\Model\User\User)
            {
                $queryCommon->leftJoin($subject->getTable() . " as user_permission_user_filter_direct_right{$strRnd}", function($join) use ($subject, $r, $strRnd)
                {
                    ...
                    $join->where("user_permission_user_filter_direct_right{$strRnd}.active_at_start", "<=", $r);
                    $join->where("user_permission_user_filter_direct_right{$strRnd}.active_at_end", ">=", $r);
            ...
            // verify direct right of subject via SubjectPermissionResource on resource
            $queryCommon->leftJoin($spr->getTable() . " as spr_filter_direct_right{$strRnd}", 
                function($join) use ($spr, $subject, $permission, $r, $strRnd)
                {
                    ...
                    $join->where("spr_filter_direct_right{$strRnd}.active_at_start", "<=", $r);
                    $join->where("spr_filter_direct_right{$strRnd}.active_at_end", ">=", $r);
                }); 
