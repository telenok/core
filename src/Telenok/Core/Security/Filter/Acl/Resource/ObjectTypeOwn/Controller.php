        ...
        foreach($subjectCollection as $subject)
        {
            ...
            if ($subject instanceof \Telenok\Core\Model\User\User)
            {
                $queryCommon->leftJoin($subject->getTable() . " as user_permission_user_filter_object_type_own{$strRnd}", 
                    function($join) use ($subject, $now, $strRnd)
                    {
                        ...
                        $join->where("user_permission_user_filter_object_type_own{$strRnd}.active_at_start", "<=", $now);
                        $join->where("user_permission_user_filter_object_type_own{$strRnd}.active_at_end", ">=", $now);
            ...
            $queryCommon->leftJoin($spr->getTable() . " as spr_filter_object_type_own_direct{$strRnd}", 
                function($join) use ($spr, $subject, $permission, $now, $strRnd)
                {
                    ...
                    $join->where("spr_filter_object_type_own_direct{$strRnd}.active_at_start", "<=", $now);
                    $join->where("spr_filter_object_type_own_direct{$strRnd}.active_at_end", ">=", $now);
